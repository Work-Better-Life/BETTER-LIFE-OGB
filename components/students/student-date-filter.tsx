...
import { getMonthOptions, getYearOptions, parseMonthValue, buildMonthValue } from "@/lib/stat-window";

const OPTIONS = ["all", "week", "month"] as const;
type FilterRange = typeof OPTIONS[number];

const LABELS: Record<FilterRange, string> = {
  all: "All",
  week: "Week",
  month: "Month",
};
...
function DropdownSelect({
  ...
  return (
    <div className="relative animate-in fade-in duration-200" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 top-full z-50 mt-1 min-w-[100px] max-h-48 overflow-y-auto rounded-md border border-border bg-surface p-1 shadow-lg">
...
export function StudentDateFilter({
  ...
  function handleSelectRange(nextRange: FilterRange) {
    const params = new URLSearchParams(searchParams);
    params.set("filterRange", nextRange);
    router.replace(`/students/${studentId}?${params.toString()}`, { scroll: false });
  }

  function handleSelectMonth(nextMonth: string) {
    const params = new URLSearchParams(searchParams);
    params.set("filterMonth", buildMonthValue(year, nextMonth));
    router.replace(`/students/${studentId}?${params.toString()}`, { scroll: false });
  }

  function handleSelectYear(nextYear: string) {
    const params = new URLSearchParams(searchParams);
    params.set("filterMonth", buildMonthValue(nextYear, month));
    router.replace(`/students/${studentId}?${params.toString()}`, { scroll: false });
  }
